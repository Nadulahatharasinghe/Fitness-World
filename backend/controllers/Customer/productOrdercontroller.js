...
export const addUsers = async (req, res) => {
  const { itemName, customerEmail, price, itemType, quantity } = req.body;
  try {
    const user = new User({ itemName, customerEmail, price, itemType, quantity });
    await user.save();
    if (!user) {
...
export const updateUser = async (req, res) => {
  ...
  try {
    const user = await User.findByIdAndUpdate(
      id,
      { itemName, customerEmail, price, itemType, quantity },
      { new: true }
    );
    if (!user) {
